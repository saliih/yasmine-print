...
(function (root, factory) {
    ...
    var Form = Backform.Form = Backbone.View.extend({
        ...
        initialize: function (options) {
            if (typeof(this.fields) == "function") this.fields = this.fields();
        ...
        render: function () {
            ...
            this.fields.each(function (field) {
                var beforeShow = field.get("beforeShow");
    ...
    var Control = Backform.Control = Backbone.View.extend({
        ...
        render: function () {
            ...
            this.$el.html(this.template(data)).addClass(field.name + '-field');
            this.updateInvalid();
    ...
    var SpacertextControl = Backform.SpacertextControl = Control.extend({
        defaults: {
            label: ""
        },
        template: _.template([
            '<% if (typeof(label) == "function") label = label(this.model);%><label class="control-label col-sm-12"><%=label%></label>',
    ...
    var TextareaControl = Backform.TextareaControl = Control.extend({
        ...
        template: _.template([
            '<label class="<%=Backform.controlLabelClassName%>"><%=label%></label>',
            '<div class="<%=Backform.controlsClassName%>">',
            '  <textarea<% if(typeof(title)!="undefined"){%> title="<%=title%>"<% } %> class="<%=Backform.controlClassName%> <%=extraClasses.join(\' \')%>" name="<%=name%>" maxlength="<%=maxlength%>" placeholder="<%-placeholder%>" <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%>><%-value%></textarea>',
            '</div>'
        ].join("\n")),
    ...
    var SelectControl = Backform.SelectControl = Control.extend({
        defaults: {
            label: "",
            extraParameters: null, // add crud for select
            title: "",
        ...
        template: _.template([
            '<label class="<%=Backform.controlLabelClassName%>"><%=label%></label>',
            '<div class="<%=Backform.controlsClassName%>">',
            '<% if (typeof(editList) != "undefined"){ %><div class="col-md-10"><% }%>',
            '  <select  <% if(typeof(title)!="undefined"){%> title="<%=title%>"<% } %> onchange="<%= change%>" class="<%=Backform.controlClassName%> <%=extraClasses.join(\' \')%>" name="<%=name%>" value="<%-value%>" <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%> >',
            '    <% if (typeof(options) == "function") options = options(this.model);for (var i=0; i < options.length; i++) { %>',
            '      <% var option = options[i]; %><% if (typeof(option.selected)== "undefined") option.selected= false; %>',
            '      <option value="<%-formatter.fromRaw(option.value)%>" <%= option.value === rawValue || option.selected ? "selected=\'selected\'" : ""%> <%=option.disabled ? "disabled=\'disabled\'" : ""%>><%-option.label%></option>',
            '    <% } %>',
            '  </select>',
            '<% if (typeof(editList) != "undefined"){ %> </div> <div class="col-md-2"><a  class="btn btn-info extrafieldparam" data-value="<%=name%>" type="button"><i class="<%=editList.icon%>"></i></a></div><% }%>',
            '</div>'
        ].join("\n")),
    ...
    var MultiSelectControl = Backform.MultiSelectControl = SelectControl.extend({
        ...
        template: _.template([
            '<label class="<%=Backform.controlLabelClassName%>"><%=label%></label>',
            '<div class="<%=Backform.controlsClassName%>">',
            '  <select  <% if(typeof(title)!="undefined"){%> title="<%=title%>"<% } %>  multiple="multiple" class="<%=Backform.controlClassName%> <%=extraClasses.join(\' \')%>" name="<%=name%>" value="<%-JSON.stringify(value)%>" <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%> style="height:<%=height%>">',
    ...
    var InputControl = Backform.InputControl = Control.extend({
        ...
        template: _.template([
            '<label class="<%=Backform.controlLabelClassName%>"><%=label%></label>',
            '<div class="<%=Backform.controlsClassName%>">',
            '  <input <% if(typeof(title)!="undefined"){%> title="<%=title%>"<% } %>  type="<%=type%>" class="<%=Backform.controlClassName%> <%=extraClasses.join(\' \')%>" name="<%=name%>" maxlength="<%=maxlength%>" value="<%-value%>" placeholder="<%-placeholder%>" <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%> />',
    ...
    var HiddenControl = Backform.HiddenControl = Control.extend({
        defaults: {
            type: "hidden",
            label: "",
            maxlength: 255,
    ...
    var NumberControl = Backform.NumberControl = Control.extend({
        defaults: {
            type: "number",
            label: "",
            title: "",
        ...
        template: _.template([
            '<label class="<%=Backform.controlLabelClassName%>"><%=label%></label>',
            '<div class="<%=Backform.controlsClassName%>">',
            '  <input  <% if(typeof(title)!="undefined"){%> title="<%=title%>"<% } %>  type="<%=type%>" class="<%=Backform.controlClassName%> <%=extraClasses.join(\' \')%>" name="<%=name%>" value="<%-value%>" placeholder="<%-placeholder%>" <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%> />',
    ...
    var PasswordControl = Backform.PasswordControl = InputControl.extend({
        defaults: {
            type: "password",
            label: "",
            title: "",
        ...
        template: _.template([
            '<label class="<%=Backform.controlLabelClassName%>"><%=label%></label>',
            '<div class="<%=Backform.controlsClassName%>">',
            '  <input <% if(typeof(title)!="undefined"){%> title="<%=title%>"<% } %>  type="<%=type%>" class="<%=Backform.controlClassName%> <%=extraClasses.join(\' \')%>" name="<%=name%>" maxlength="255" value="<%-value%>" placeholder="<%-placeholder%>" <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%> />',

            '</div>'
        ].join("\n")),

    });
    var EmailControl = Backform.EmailControl = InputControl.extend({
        defaults: {
            type: "email",
            label: "",
            title: "",
        ...
        template: _.template([
            '<label class="<%=Backform.controlLabelClassName%>"><%=label%></label>',
            '<div class="<%=Backform.controlsClassName%>">',
            '  <input <% if(typeof(title)!="undefined"){%> title="<%=title%>"<% } %>  type="<%=type%>" class="<%=Backform.controlClassName%> <%=extraClasses.join(\' \')%>" name="<%=name%>" maxlength="255" value="<%-value%>" placeholder="<%-placeholder%>"  />',
    ...
    var RadioControl = Backform.RadioControl = InputControl.extend({
        ...
        template: _.template([
            ...
            '      <input <% if(typeof(title)!="undefined"){%> title="<%=title%>"<% } %>  type="<%=type%>" class="<%=extraClasses.join(\' \')%>" name="<%=name%>" value="<%-formatter.fromRaw(option.value)%>" <%=rawValue == option.value ? "checked=\'checked\'" : ""%> <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%> /> <%-option.label%>',
            '    </label>',
            '  <% } %>',
    ...
    // Requires the SummerNote to work.
    var WysiwigControl = Backform.WysiwigControl = TextareaControl.extend({
        ...
        render: function () {
            TextareaControl.prototype.render.apply(this, arguments);
            var thisbox = this;
            //this.$el.find("textarea").after( "<textarea id='summernote'></textarea>" );
            //this.$el.find("textarea").hide();
            this.$el.find("textarea").summernote({
                onBlur: function (e) {
                    thisbox.$el.find("textarea").eq(0).val(thisbox.$el.find("textarea").code());
                    thisbox.$el.find("textarea").eq(0).trigger("change");
                },
...
// Requires the Bootstrap Colorpicker to work.
    var ColorpickerControl = Backform.ColorpickerControl = InputControl.extend({
        defaults: {
            type: "text",
            label: "",
            options: {},
        ...
        render: function () {
            var thisView = this
            InputControl.prototype.render.apply(this, arguments);
            this.$el.find("input").colorpicker().on('hidePicker', function (ev) {
                thisView.onChange(ev);
    ...
    var ButtonControl = Backform.ButtonControl = Control.extend({
        ...
        template: _.template([
            '<label class="<%=Backform.controlLabelClassName%>">&nbsp;</label>',
            '<div class="<%=Backform.controlsClassName%>">',
            '  <button <% if(typeof(title)!="undefined"){%> title="<%=title%>"<% } %>  type="<%=type%>" name="<%=name%>" class="btn <%=extraClasses.join(\' \')%>" <%=disabled ? "disabled" : ""%> ><%=label%></button>',
